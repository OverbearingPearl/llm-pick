    ...
    (let ((llm-pick--last-query nil))
      (cl-letf (((symbol-function 'read-string)
        ...
        (ert-info ("Every word of the line has a reader")
          (should (equal (llm-pick--read-args "Report query: ")
    ...
    (let ((llm-pick--last-query nil))
      (cl-letf (((symbol-function 'read-string)
        ...
        (ert-info ("A misspelled word must not be dropped in silence")
          (should-error (llm-pick--read-args "Report query: ")
                        :type 'llm-pick-error))))))
...
(ert-deftest llm-pick-report-test-a-report-asks-nothing-without-a-prefix ()
  (llm-pick-report-test--with-fixtures
    (ert-info ("Without a prefix argument the interactive spec reads nothing")
      (let ((asked nil))
        (cl-letf (((symbol-function 'llm-pick--read-args)
...
(ert-deftest llm-pick-report-test-query-completion-offers-words-then-values ()
  (with-temp-buffer
    (ert-info ("An empty line offers every word")
      (should (equal (llm-pick--query-candidates (point))
    ...
    (ert-info ("A category has nothing to offer: llm-pick picks it")
      (erase-buffer)
      (insert "category ")
      (should-not (llm-pick--query-candidates (point))))
    ...
    (ert-info ("A column list offers the fields plus the bar")
      (erase-buffer)
      (insert "columns ")
      (should (member "bar" (llm-pick--query-candidates (point))))
      (should (member "or-out" (llm-pick--query-candidates (point)))))
    (ert-info ("A number still gets a value to start from")
      (erase-buffer)
      (insert "budget ")
      (should (llm-pick--query-candidates (point))))))
...
(ert-deftest llm-pick-report-test-query-minibuffer-map-tab-completes ()
  (ert-info ("TAB in the query minibuffer completes, it does not self-insert")
    (should (eq (lookup-key llm-pick--query-minibuffer-map "\t")
                #'llm-pick--query-complete))))
...
(ert-deftest llm-pick-report-test-query-word-help-covers-every-word ()
  (ert-info ("Every word a query accepts also explains itself")
    (dolist (entry llm-pick--query-words)
      (should (stringp (car entry)))
      (should (keywordp (nth 1 entry)))
      (should (stringp (nth 2 entry)))
      (should (llm-pick--query-key (car entry)))))
  (ert-info ("Every word but `category' offers candidates for its value")
    ;; Leaving `category' out scores a model by the best of its
    ;; categories, so there is no list to choose from and none is shown.
    (dolist (entry llm-pick--query-words)
      (should (or (llm-pick--query-values (nth 1 entry))
